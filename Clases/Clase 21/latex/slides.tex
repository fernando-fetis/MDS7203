...
\subtitle{Clase 21: Modelos de difusión (parte III)}
...
\begin{frame}{Idea general}
    ...
    Un método usual consiste en sumar el embedding temporal (asociado al tiempo de denoising $t\in\{1,\ldots,T\}$) y el embedding de la condición $y\in\R^C$, para luego inyectar esta suma en la red neuronal mediante \textbf{modulaciones tipo FiLM}.
\end{frame}
...
\begin{frame}{U-Net}
    ...
    \insertimage{cross_attn_unet}{0.5}
    \vspace{-0.5cm}
    Notar que se usan dos representaciones de $y$: un \textbf{embedding global} sumado al embedding temporal e inyectado por FiLM; y la \textbf{secuencia} $\{e_1,\ldots,e_L\}$, inyectada por atención cruzada.
\end{frame}
...
\begin{frame}
    ...
    \large{Clase 21: Modelos de difusión (parte III)}
\end{frame}
